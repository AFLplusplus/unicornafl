...
use unicornafl::{InsnSysX86, RegisterARM, RegisterMIPS, RegisterPPC, RegisterX86, Unicorn};
...
fn emulate_x86() {
    ...
    let mut emu = unicornafl::Unicorn::new(Arch::X86, Mode::MODE_32, 0)
...
fn x86_code_callback() {
    ...
    let mut emu = unicornafl::Unicorn::new(Arch::X86, Mode::MODE_32, ())
        .expect("failed to initialize unicorn instance");
    assert_eq!(emu.mem_map(0x1000, 0x4000, Permission::ALL), Ok(()));
    assert_eq!(emu.mem_write(0x1000, &x86_code32), Ok(()));
...
fn x86_insn_sys_callback() {
    ...
    let mut emu = unicornafl::Unicorn::new(Arch::X86, Mode::MODE_64, ())
        .expect("failed to initialize unicorn instance");
    assert_eq!(emu.mem_map(0x1000, 0x4000, Permission::ALL), Ok(()));
...
fn emulate_arm() {
    ...
    let mut emu = unicornafl::Unicorn::new(Arch::ARM, Mode::THUMB, ())
        .expect("failed to initialize unicorn instance");
...
fn emulate_mips() {
    ...
    let mut emu = unicornafl::Unicorn::new(Arch::MIPS, Mode::MODE_32, ())
        .expect("failed to initialize unicorn instance");
    assert_eq!(emu.mem_map(0x1000, 0x4000, Permission::ALL), Ok(()));
...
fn emulate_ppc() {
    ...
    let mut emu = unicornafl::Unicorn::new(Arch::PPC, Mode::PPC32, ())
        .expect("failed to initialize unicorn instance");
    assert_eq!(emu.mem_map(0x1000, 0x4000, Permission::ALL), Ok(()));
