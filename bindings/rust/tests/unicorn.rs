...
use unicornafl::unicorn_const::{Mode, Arch, Permission, MemType, HookType, SECOND_SCALE, uc_error};
...
fn x86_insn_sys_callback() {
    ...
    let mut unicorn = unicornafl::Unicorn::new(Arch::X86, Mode::MODE_64, 0).expect("failed to initialize unicorn instance");
    let mut emu = unicorn.borrow();
    assert_eq!(
        emu.mem_map(0x1000, 0x4000, Permission::ALL),
        Ok(())
...
fn emulate_arm() {
    ...
    let mut unicorn = unicornafl::Unicorn::new(Arch::ARM, Mode::THUMB, 0).expect("failed to initialize unicorn instance");
    let mut emu = unicorn.borrow();
...
fn emulate_mips() {
    ...
    let mut unicorn = unicornafl::Unicorn::new(Arch::MIPS, Mode::MODE_32, 0).expect("failed to initialize unicorn instance");
    let mut emu = unicorn.borrow();
    assert_eq!(
        emu.mem_map(0x1000, 0x4000, Permission::ALL),
        Ok(())
...
fn x86_context_save_and_restore() {
    for mode in vec![Mode::MODE_32, Mode::MODE_64] {
        ...
        let mut unicorn = unicornafl::Unicorn::new(Arch::X86, mode, 0).expect("failed to initialize unicorn instance");
        let mut emu = unicorn.borrow();
        assert_eq!(
            emu.mem_map(0x1000, 0x4000, Permission::ALL),
            Ok(())
        ...
        let mut unicorn2 = unicornafl::Unicorn::new(Arch::X86, mode, 0).expect("failed to initialize unicorn instance");
